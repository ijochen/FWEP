...
BEGIN
    ...
    DROP TABLE IF EXISTS warehouse.fweps_inventory_position_count;

    CREATE TABLE warehouse.fweps_inventory_position_count as 
    ...
    FROM (
        ...
        select * from warehouse.fweps_inventory_position
        union
        --original table from first ETL
        SELECT * FROM warehouse.fweps_inventory_position_interim
        UNION
        --daily refreshed table of what's currently in stock at each branch
        SELECT *, CURRENT_DATE trans_date FROM warehouse.fweps_inventory_position_incremental
    ) t1;


    --create a table that ranks the duplicates by dates to see which was the original record
    DROP TABLE IF EXISTS warehouse.fweps_inventory_position_rank;

    CREATE TABLE warehouse.fweps_inventory_position_rank as 
    SELECT *, dense_rank() OVER( PARTITION BY location_id, item_id, count order by count, trans_date asc) rank
    FROM (
        SELECT * FROM warehouse.fweps_inventory_position_count
    ) t2;

    --delete the duplicates before inserting it into the production table
    DELETE FROM warehouse.fweps_inventory_position_rank 
    WHERE count = 2 and rank = 2;

    --delete all rows before inserting the unique/distinct records
    TRUNCATE TABLE warehouse.fweps_inventory_position;

    --reinsert distinct records
    INSERT INTO warehouse.fweps_inventory_position
    ...
    FROM warehouse.fweps_inventory_position_rank;

    --drop these tables to save on cost/space
    DROP TABLE IF EXISTS warehouse.fweps_inventory_position_count, warehouse.fweps_inventory_position_rank ;

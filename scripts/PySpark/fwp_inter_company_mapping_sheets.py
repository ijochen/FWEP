...
s3_df_trans = spark.read.format("csv") \
   .option("header", "true") \
   .load("s3a://fwp-bucket/Inter Company Transfer Mapping Data.xlsx")
...
s3_df_trans.write.jdbc(url=url, table="warehouse.fwp_intercompany_transfer_mapping_data", mode=mode, properties=properties)

# Regional Mgmt Mapping
s3_df_mgmt = spark.read.format("csv") \
   .option("header", "true") \
   .load("s3a://fwp-bucket/Regional Mgmt Mapping.xlsx")
s3_df_mgmt.write.jdbc(url=url, table="warehouse.fwp_regional_mgmt_mapping_data", mode=mode, properties=properties)

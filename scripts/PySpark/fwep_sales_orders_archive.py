...
pep_query = """(
    ...
    where INV_DATE >= '2019-01-01' and t1.PROD_NUM not in ('C','CSB','CS','CI','CN','MN') and c.CUST_NUM not like '%CLOSED'
    group by 
        t3.CUST_NUM, 
        t3.ORD_DATE, 
        t3.ORD_NUM, 
...
fwp_query = """(
    ...
    where year(invoice_date) >= 2019 and gl_revenue_account_no like '4000%' and il.order_no is not null-- and item_desc not like '%TAX'
    group by 
        ih.customer_id, 
        ih.bill2_name, 
        order_date, 

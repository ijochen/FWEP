...
pep_query = """(
    SELECT --DISTINCT --TOP(100)
        ...
        PL.VPROD_NUM,
        PL.UN_MEAS,
        ISNULL(PL.ORD_QTY,0) ORD_QTY,
        ISNULL(PL.REC_QTY,0) REC_QTY,
        ISNULL(PL.GRS_COST,0) GRS_COST,
        ISNULL(PL.EXT_AMT,0) EXT_AMT,
        PH.CENTRAL_WHSE_NUM,
        CASE	 
    ...
    LEFT JOIN PRELUDE.DBO.PRODUCT_IJO P ON PH.CO_NUM = P.CO_NUM AND PL.PROD_NUM = P.PROD_NUM
    LEFT JOIN PRELUDE.DBO.CATEGORY_IJO CA ON PH.CO_NUM = CA.CO_NUM AND P.PLINE_NUM = CA.PLINE_NUM
    LEFT JOIN PRELUDE.DBO.VEND_IJO V ON PH.CO_NUM = V.CO_NUM AND PH.VEND_NUM = V.VEND_NUM 
    LEFT JOIN PRELUDE.DBO.BUYER_NF B ON PH.CO_NUM = B.CO_NUM AND PH.BUYER_NUM = B.BUYER_NUM
    LEFT JOIN PRELUDE.DBO.USER_ID_NF U ON PH.BUYER_NUM = U.USER_NUM
    WHERE YEAR(PH.PO_DATE) = 2021 AND PH.CO_NUM = '001' 
		--AND P.CO_NUM = '001' AND CA.CO_NUM = '001' 
		/*PL.PROD_NUM NOT IN ('C','CSB','CS','CI','CN','CP','MN')  AND */
...
pep_df.write.jdbc(url=url, table="procurement.pep_purchase_orders_2021", mode=mode, properties=properties)
